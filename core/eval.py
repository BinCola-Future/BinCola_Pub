    ...
    for func_key in funcs.keys():
        ...
        for src_opt, src_func in func_data.items():
            ...
            while n < negative_num:
                while True:
                    func_tn_key = random.choice(g_func_keys)  # (package, bin_name, func_data["name"])
...
def do_eval(opts):
    ...
    for config_fname in tqdm(config_fname_list):
        ...
        train_tp_pairs, train_tn_pairs = create_train_pairs(train_funcs, dst_options, optionidx_map, opts.negative_num)
        train_data_loader = CreateDataLoader(train_tp_pairs, train_tn_pairs, opts.batch_size, device)
        
        valid_tp_pairs, valid_tn_pairs = create_train_pairs(valid_funcs, dst_options, optionidx_map, opts.negative_num)
        valid_data_loader = CreateDataLoader(valid_tp_pairs, valid_tn_pairs, opts.batch_size, device)

        test_tp_pairs, test_tn_pairs = create_train_pairs(test_funcs, dst_options, optionidx_map, opts.negative_num)
        test_data_loader = CreateDataLoader(test_tp_pairs, test_tn_pairs, opts.batch_size, device)

        logger.info("# of Train Pairs: %d", len(train_tp_pairs) + len(train_tn_pairs)*opts.negative_num)
        ...
        if opts.test_type == 'with_dl':
            ...
            for i, data in enumerate(test_data_loader):
                ...
                pos_origin.extend(pos.cpu().detach().numpy())
                ...
                pos_out_array = pos.cpu().detach().numpy()
            ...
            for i, data in enumerate(test_data_loader):
                ...
                pos_out.extend(pos_out_array)
                neg_out.extend(neg_out_array)
                similarity = F.cosine_similarity(torch.cat((src,src),0), torch.cat((pos,neg[:,0,:]),0), dim=1, eps=1e-8)
                pred = similarity.cpu().detach().numpy()
        ...
        for poolsize in poolsize_list:
            ...
            topk_list,MRR = calc_topK(src_out+src_out, pos_out+neg_out, lable_all, k_list, poolsize, opts.sim_method)  # 采样test+val个函数计算topk
